...
<table id="rounded-corner" summary="2007 Major IT Companies' Profit">
    ...
    <tbody>
        <?php
        $query = $this->db->query("select * from Bookings ORDER by Booking_ID ASC");
        ...
        <tr>
            ...
            <?php
                $id1 = $row->guest_id;
                $query2= $this->db->query("select * from Guests where Guest_ID = '$id1'");
                $row2 = $query2->row();
            ?>
            <td><?php
                echo $row2->Guest_Name;
            ?></td>
            <?php
                $id3 = $row->Employee_ID;
                $query3= $this->db->query("select * from Staff where Employee_ID = '$id3'");
                $row3 = $query3->row();
            ?>
            <td><?php
                echo $row3->Employee_Name;
            ?></td>
            <?php
                $id2 = $row->Booking_ID;
            ...
            <td><form method="post" action="<?php echo base_url() . "main/delete_booking"?>">
                    <input type="text" style="display:none" name="Booking_ID" value="<?php echo $row->Booking_ID ?>">
